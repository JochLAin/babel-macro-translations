...
class TranslateMacro extends abstract_1.default {
    ...
    buildNodeWithIdentifierLocale(node) {
        ...
        const translateMethodIdentifier = (0, helper_module_imports_1.addNamed)(node.parentPath, 'translate', '@jochlain/translations', { hintedName: `translate_$${++counter}` });
    ...
    getArguments(node) {
        ...
        if (this.isLocaleLiteral(node)) {
            ...
            const catalog = { [locale]: catalogs?.[locale]?.[domain] ? (0, translations_1.getCatalogValue)(catalogs[locale][domain], message) : message };
