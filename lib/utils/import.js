...
const helper_module_imports_1 = __importStar(require("@babel/helper-module-imports"));
...
const searchModule = (node, moduleName, name, isDefault = false) => {
    ...
    const isMod = (0, helper_module_imports_1.default)(programPath);
...
const getProgramPath = (node) => {
    if (!programMap.has(node)) {
        ...
        if (!programPath) {
            throw node.parentPath?.buildCodeFrameError(`Can't reach program node`, babel_plugin_macros_1.MacroError);
        }
